...
namespace EventManagementSystem.DataAccessLayer.Repositories
{
    public class ParticipantEventRepository : IParticipantEventRepository
    {
        ...
        public async Task<bool> RegisterAsync(ParticipantEventDetails entry)
        {
            ...
            {
                _logger.LogError(ex,
                    "Error registering participant {Email} for event {EventId}",
        ...
        public async Task<ParticipantEventDetails?> GetByIdAsync(Guid registrationId)
        {
            return await _context.ParticipantEvents
                .Include(p => p.Event)
        ...
        public async Task<bool> UpdateAttendanceAsync(Guid id, bool isAttended)
        {
            ...
            {
                _logger.LogError(ex,
                    "Error updating attendance for registration {Id}", id);
                return false;
            }
        }

        public async Task<IEnumerable<ParticipantEventDetails>> GetByEventIdAsync(Guid eventId)
        {
            return await _context.ParticipantEvents
